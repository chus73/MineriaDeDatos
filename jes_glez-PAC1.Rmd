...
> cada fase.
>
> Esta metodología puede ser consultada en el siguiente documento:
> <https://www.the-modeling-agency.com/crisp-dm.pdf>, y consta de seis
> fases que se van realizando cíclicamente. Por otro lado, si en una de
> las fases se detectasen carencias, se permite la realimentación hacia
> fases previas para poder solucionarlas, tal y como se puede observar
> en diagrama que se muestra a continuación (*Extraído del documento
> crisp-dm 1.0 pag. 12, enlace superior*):
>
...
> representará. Como entregables a cumplir tenemos **el plan de
> proyecto** preliminar necesario para alcanzar el resultado esperado y
> los objetivos mínimos a cumplir.
>
...
>     carreteras estatales por encima de un 70%.
>
...
> proyecto. En este sentido, se han definido los [entregables
> mínimos]{.ul} que aparecen en la tabla superior.
>
...
>
> Se establecen **puntos de control** con negocio con una periodicidad
> [mínima]{.ul} a la finalización de cada fase.
>
> ### Data Understanding
>
...
> información oculta o los subconjunto interesante de datos a tratar,
> que permitan tener una visión desde diferentes perspectivas. Un punto
...
> que el proyecto tenga sentido la **ventana temporal de entrega** del
> flujo tiene que ser en tiempo ["quasi" real]{.ul}. Solamente de esta
> manera se tendrá la suficiente antelación como para poder actuar antes
> de que la fotografía registrada esté caducada.
>
...
>
> -   **fnlwgt**: Si nos fijamos, existe una diferencia muy alta entre
>     los valores que toma el tercer intercuartil y el valor máximo.
>     Esto hace que, o bien tengamos que tratarlos como outliers,
>     eliminándolos o sustituyéndolos por otro valor para arreglar la
>     descompensación, o bien no tengamos en cuenta esta variable. En
>     este estudio se a optado por la segunda opción, al no tener muy
>     claro el propósito de la variable.
>
...
>     Masters, ...". Para esto nos serviremos más adelante de la
...
>     distanciados entre ambas. Esto se puede ver también si miramos los
>     valores máximos y los comparamos con el tercer intercuartil.
>     Podríamos pasarlas a categóricas binarias para expresar las
>     pérdidas y ganancias. No obstante, se ha decidido prescindir de
>     ellas para este estudio.
>
...
>     la variable. Vemos además que presenta el carácter "?" para los
...
En el gráfico superior podemos observar que la proporción de personas
con estudios superiores que ganan por encima de los 50k es de
aproximadamente la mitad. En cambio, para las personas con estudios
medios se ve una gran desproporción.

Vamos a mirar dentro del grupo de estudios medios si destaca alguna
clase sobre otra.
...
Ampliaremos el análisis añadiendo la variable race al estudio.

```{r}
ggplot(data=totalData,aes(x=sex,fill=income))+geom_bar(position="fill") + facet_wrap(~race)
```

Se observa como continúa la desproporción entre mujeres y hombres
...
superiores**. Sorprende que en el caso de la **población negra** esto
**no sucede**, siendo la que tiene estudios medios la predominante.
...
Es interesante notar que para ganar más de 50k tienes que trabajar más
de las 40h semanales, que corresponde con la jornada máxima permitida en
Europa.
...
```{r}
ggplot(data=totalData,aes(x=hours.per.week,fill=income)) + geom_histogram(binwidth =15,position="fill") + facet_wrap(~race) + geom_line(y = 0.25)
```
...
A lo largo de este estudio realizado en EEUU, hemos visto como la
diferentes aspectos sociales pueden determinar si una persona puede
tener un mejor salario. Para ello se ha trazado una línea fijada en los
50.000 \$ y después de realizar el tratamiento de los datos se ha
procedido a comparar las diferentes variables entre ellas.

Se puede destacar como una educación superior y el trabajar por encima
de las 40 h semanales son determinantes para estar por encima del límite
de ingresos fijados. Por otro lado, se destaca la enorme diferencia
salarial de 1:3 de mujeres respecto a hombres. También es determinante
el lugar de donde procedes. En efecto, hay una diferencia grande entre
las personas cuya procedencia no es nativa norteamericana y proceden de
asia o de otras partes del mundo. En este caso, la migración asiática
tiene más probabilidades de estar por encima del límite de ganancia
fijado. Eso no pasa en el caso de otras razas, salvo la blanca.

En definitiva, el retrato típico que ofrece el estudio sobre el
ciudadano norteamericano que gana por encima de los 50k, refleja a una
**persona varón, de raza blanca, con estudios superiores y que trabaja
más de 40 h/semanales**.
